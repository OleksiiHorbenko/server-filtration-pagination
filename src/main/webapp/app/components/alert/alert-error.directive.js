(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('dynamicFiltrationPaginationApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-dynamicFiltrationPaginationApp-error');
                var entityKey = httpResponse.headers('X-dynamicFiltrationPaginationApp-params');
                if (errorHeader) {
